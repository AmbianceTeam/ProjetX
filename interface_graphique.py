...
def dessiner_terrain(w,Graphe):
    ...
    for i in range (listInfoTerrain[4]): # dessin des cellules
        print('here')
        w.create_circle(listCellules[i].x*scaleX+offsetX, listCellules[i].y*scaleY+offsetY, (listCellules[i].radius)/4, fill=listCouleurs[listCellules[i].couleur], outline="#FCFCFC", width=2)
